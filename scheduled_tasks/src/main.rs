...
async fn main() -> anyhow::Result<()> {
    ...
    let chats = sqlx::query!("SELECT DISTINCT id FROM chats")
        .fetch_all(&pool)
        .await
        .unwrap_or(vec![]);
...
async fn send_polls(pool: &PgPool, chat_id: i64, bot: &teloxide::Bot) -> anyhow::Result<()> {
...
async fn stop_poll(pool: &PgPool, bot: &teloxide::Bot) -> Result<(), Error> {
